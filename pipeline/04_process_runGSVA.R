...
#------------------- 重新绘图函数 -------------------#
replot_gsva_bars <- function(gsva_scores_file, color_palette_file, top_n = 10, output_plot_dir = "results/gsva") {
  # 加载数据
  gsva_scores <- read_csv(gsva_scores_file)
  color_palette <- readRDS(color_palette_file)
  
  # 绘图
  p <- plot_gsva_bars_prism(gsva_scores, color_palette, top_n, output_plot_dir)
  
  # 保存自定义配色的图片
  ggsave(
    file.path(output_plot_dir, "custom_gsva_barplot.pdf"),
    plot = p,
    width = 12,
    height = max(6, top_n * 0.5)
  )
  
  ggsave(
    file.path(output_plot_dir, "custom_gsva_barplot.png"),
    plot = p,
    width = 12,
    height = max(6, top_n * 0.5),
    dpi = 300
  ...
  log_success("Custom GSVA barplot created")
  return(p)
}

#------------------- 收集通路ID的函数 -------------------#
collect_pathway_ids <- function(enrich_dir, top_n = 10) {
  log_info(glue("Collecting top {top_n} pathway IDs per cluster from enrichment results"))
  cluster_dirs <- list.dirs(enrich_dir, recursive = FALSE)
  pathway_df <- tibble()
  ...
  for (dir in cluster_dirs) {
    ...
    enrich_file <- file.path(dir, "Enrich_summary.csv")
    if (file.exists(enrich_file)) {
      enrich_data <- read_csv(enrich_file, show_col_types = FALSE)
      if ("ID" %in% colnames(enrich_data) && nrow(enrich_data) > 0) {
        # 按p.adjust排序并取前top_n个通路
        cluster_pathways <- enrich_data %>%
          arrange(p.adjust) %>%
          slice_head(n = top_n) %>%
          mutate(Cluster = cluster_name) %>%
          dplyr::select(Cluster, ID, Description)
        
        pathway_df <- bind_rows(pathway_df, cluster_pathways)
      }
    }
  }
  
  if (nrow(pathway_df) == 0) {
    log_error("No pathway IDs found in enrichment results")
    stop("Pathway IDs not found")
  }
  
  # 保存结果表格
  output_file <- file.path(enrich_dir, "top_pathways_per_cluster.csv")
  write_csv(pathway_df, output_file)
  
  log_success(glue("Collected {nrow(pathway_df)} pathway IDs from {length(unique(pathway_df$Cluster))} clusters"))
  return(pathway_df)
}
...
if (exists("snakemake")) {
  ...
  #加载分析通路
  pathway_df <- collect_pathway_ids(
    enrich_dir = snakemake@input$enrich_dirs, 
    top_n = config$top_n
  )
